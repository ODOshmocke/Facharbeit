...
engine = create_engine(sqlite_url, connect_args=connect_args, pool_size=30, max_overflow=30)
...
async def update_todo_item(
    ...
    # 1. Todo suchen und sicherstellen, dass es dem eingeloggten User gehört
    statement = select(TodoItemDB).where(
        TodoItemDB.id == item_id,
        TodoItemDB.owner_id == current_user.id
    )
    ...
    # 2. Nur die Felder aktualisieren, die im Request mitgeschickt wurden
...
async def delete_todo_item(
    ...
    return {"ok": True, "message": f"Todo mit ID {item_id} erfolgreich gelöscht"}
